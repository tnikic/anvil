...
Description, command list, and contextual tips — text that appears in both the dashboard and the skill — live in an `internal/content` package as Go structs. Both `runHome` (dashboard rendering) and `cmd/generate` (skill generator) import this package as the single source of truth.
